...
Account: gse.frank.li@gmail.com
Password: EmpireSX@2025
"""
...
for endpoint in endpoints:
    ...
    try:
        ...
        # API-Sports usually puts data inside "response"
        ...
        df.to_csv(f"f1_data_csv/{filename}", index=False)

        print(f"✅ Saved {endpoint} → {filename} ({len(df)} rows)")
...
for endpoint in endpoints:
    ...
    try:
        ...
        with open(f"f1_data/{filename}", "w") as f:
